...
<script type="text/javascript">
    ...
    function LoadTierDetails(iTierID) {
        ...
        $.ajax({
            ...
            url: '<%= ResolveUrl("~/GlobalPriceList/GetTierByID") %>',
    ...
    function DoesBandExist(iBand) {
        ...
        ShowLoadingBox();
        $.ajax({
            url: '<%= ResolveUrl("~/GlobalPriceList/DoesBandExist") %>',
            data: { "iBand": iBand },
            type: "POST",
            cache: false,
            timeout: 6000000,
    ...
    function DoesBandExistOnUpdate(iTierID, iBand) {
        ...
        ShowLoadingBox();
        $.ajax({
            url: '<%= ResolveUrl("~/GlobalPriceList/DoesBandExistOnUpdate") %>',
    ...
    function AddPriceList() {
        ...
        if (Validate()) {
            ...
            ShowLoadingBox();
            $.ajax({
                url: '<%= ResolveUrl("~/GlobalPriceList/AddPriceList") %>',
                ...
                success: function (result) {
                    ...
                    if (result == "true") {
                        ...
                        $('#errorMsg').show().text('Error occurred while adding Price List Tier. Please try again.');
                    }
                },  // ends success        
                error: function (httpRequest, msg) {
    ...
    function UpdatePriceList() {
        ...
        if (Validate()) {
            ...
            ShowLoadingBox();
            $.ajax({
                url: '<%= ResolveUrl("~/GlobalPriceList/UpdatePriceList") %>',
                ...
                success: function (result) {
                    ...
                    if (result == "true") {
                        ...
                        $('#errorMsg').show().text('Error occurred while updating Price List Tier. Please try again.');
                    }
                },  // ends success        
                error: function (httpRequest, msg) {
                    HideLoadingBox();
